...
export async function userHandler(rest: REST, clientId: string, commandLevel: string, guildId?: string) {
  let commands: APIApplicationCommand[];

  if (commandLevel === 'global-command') {
    commands = await fetchGlobalApplicationCommands(rest, ApplicationCommandType.User, clientId);
  } else {
    commands = await fetchGuildApplicationCommands(rest, ApplicationCommandType.User, clientId, guildId!);
  }
